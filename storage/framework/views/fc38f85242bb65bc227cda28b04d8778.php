...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-slate-400">Kelola pengajuan izin dan sakit dari mahasiswa</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm dark:shadow-slate-900/50 border border-gray-100 dark:border-slate-700">
        <form method="GET" class="flex flex-col sm:flex-row gap-3">
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari NIM atau nama..."
                   class="flex-1 px-4 py-2 rounded-lg border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm focus:border-uin-green focus:ring-1 focus:ring-uin-green/20 outline-none">
                        ...
                        <tr class="hover:bg-gray-50 dark:bg-slate-700">
                            ...
                            <td class="px-5 py-3">
                                <?php if($p->status == 'pending'): ?>
                                    <div x-data="{ showModal: false }" class="flex gap-2">
                                        
                                        <button @click="$refs.approveForm<?php echo e($p->id_pengajuan); ?>.submit()"
                                                class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-medium hover:bg-emerald-200"
                                                title="Setujui">
                                            <i class="fas fa-check"></i>
                                        </button>
                                        <form x-ref="approveForm<?php echo e($p->id_pengajuan); ?>" action="<?php echo e(route('admin.pengajuan.approve', $p->id_pengajuan)); ?>" method="POST" class="hidden">
                                            <?php echo csrf_field(); ?>
                                        </form>
                                        ...
                                        <div x-show="showModal" x-transition @click.away="showModal = false" x-cloak
                                             class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                                            <div class="bg-white dark:bg-slate-800 rounded-xl p-6 w-full max-w-md shadow-xl" @click.stop>
                                                <h4 class="text-lg font-bold text-gray-800 dark:text-white mb-3">Tolak Pengajuan</h4>
                                                <form action="<?php echo e(route('admin.pengajuan.reject', $p->id_pengajuan)); ?>" method="POST">
                                                    <?php echo csrf_field(); ?>
                                                    <div class="mb-4">
                                                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Catatan (opsional)</label>
                                                        <textarea name="catatan_admin" rows="3" placeholder="Alasan penolakan..."
                                                                  class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm outline-none focus:border-red-400"></textarea>
                                                    </div>
                                                    <div class="flex justify-end gap-2">
                                                        <button type="button" @click="showModal = false"
                                                                class="px-4 py-2 text-sm text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg">
                                                            Batal
                                                        </button>
                                                        <button type="submit"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\ghifa's court\web-presensi-feb\resources\views/admin/pengajuan/index.blade.php ENDPATH**/ ?>