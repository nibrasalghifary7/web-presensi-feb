...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h3 class="text-lg font-bold text-gray-800">Daftar Akun Pengguna</h3>
            <p class="text-sm text-gray-500">Kelola akun login semua pengguna sistem</p>
        </div>
    ...
    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <form method="GET" class="flex flex-col sm:flex-row gap-3">
            ...
            <select name="role" class="px-4 py-2 rounded-lg border border-gray-200 text-sm">
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Terdaftar</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-5 py-3 text-sm text-gray-500"><?php echo e($users->firstItem() + $index); ?></td>
                            <td class="px-5 py-3 text-sm font-medium text-gray-800"><?php echo e($u->username); ?></td>
                            <td class="px-5 py-3 text-sm text-gray-800"><?php echo e($u->name); ?></td>
                            <td class="px-5 py-3 text-sm text-gray-600"><?php echo e($u->email ?? '-'); ?></td>
                            <td class="px-5 py-3">
                                <?php if($u->role === 'admin'): ?>
                            ...
                            <td class="px-5 py-3 text-sm text-gray-600"><?php echo e($u->created_at ? $u->created_at->translatedFormat('d M Y') : '-'); ?></td>
                            <td class="px-5 py-3">
    ...
    <div id="modalTambah" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <h3 class="text-lg font-bold text-gray-800 mb-4">Tambah Akun</h3>
            <form method="POST" action="<?php echo e(route('admin.users.store')); ?>" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="role" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-uin-green outline-none">
    ...
    <div id="modalEdit" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <h3 class="text-lg font-bold text-gray-800 mb-4">Edit Akun</h3>
            <form id="formEdit" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Role</label>
                    <select id="edit_role" name="role" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-uin-green outline-none">
                        <option value="mahasiswa">Mahasiswa</option>
                        <option value="dosen">Dosen</option>
                        <option value="admin">Admin</option>
                    </select>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\web-presensi-feb\resources\views/admin/users/index.blade.php ENDPATH**/ ?>