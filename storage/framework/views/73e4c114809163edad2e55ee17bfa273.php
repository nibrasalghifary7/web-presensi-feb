...
<div class="space-y-6">
    ...
    <?php if($pendingCount > 0): ?>
        <div class="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-semibold text-amber-800 dark:text-amber-300"><?php echo e($pendingCount); ?> pengajuan menunggu persetujuan Anda</p>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                    <?php $__empty_1 = true; $__currentLoopData = $pengajuans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 <?php echo e($p->status == 'pending' ? 'bg-amber-50/30 dark:bg-amber-500/5' : ''); ?>">
                            ...
                            <td class="px-5 py-3">
                                <?php if($p->status == 'pending'): ?>
                                    <div class="flex gap-2">
                                        ...
                                        <form action="<?php echo e(route('dosen.pengajuan.approve', $p->id_pengajuan)); ?>" method="POST"
                                              onsubmit="return confirm('Setujui pengajuan <?php echo e($p->jenis); ?> dari <?php echo e($p->mahasiswa->nama ?? $p->nim); ?>?')">
                                            <?php echo csrf_field(); ?>
                                            <button type="submit"
                                        ...
                                        <form action="<?php echo e(route('dosen.pengajuan.reject', $p->id_pengajuan)); ?>" method="POST"
                                              onsubmit="return confirm('Tolak pengajuan <?php echo e($p->jenis); ?> dari <?php echo e($p->mahasiswa->nama ?? $p->nim); ?>?')">
                                            <?php echo csrf_field(); ?>
