{{--
    Profil Dosen - Dual Theme
    Style sama dengan mahasiswa profile
--}}
@extends('layouts.app')
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden glass">
        ...
        <div class="bg-gradient-to-r from-primary to-primary-light px-6 py-8 text-center dark:from-aurora-glow dark:to-aurora-glow-tertiary">
            <div class="inline-flex items-center justify-center w-20 h-20 bg-white rounded-full shadow-lg mb-3">
                <span class="text-3xl font-bold text-primary dark:text-aurora-glow">{{ strtoupper(substr($dosen->nama ?? $user->name, 0, 1)) }}</span>
            </div>
            <h2 class="text-2xl font-bold text-white">{{ $dosen->nama ?? $user->name }}</h2>
            <p class="text-white/80 text-sm mt-1">{{ $dosen->nidn ?? '-' }}</p>
